var SocialNationItsmeUserReport = {
	...
	updatePendingUserRecursiveAjaxCall: function(params){
		...
		var mThis = this;
		mThis.updateUuidToUpdateLog({text:"\nUPDATE USERS: Inizio aggiornamento utente. (uuid="+uuid+").\n"});
		jQuery.post(
			socialNationItsmeAjax.ajaxurl, 
			data, 
			function(response) {
				if(response.success){
					...
					mThis.updateUuidToUpdateLog({text:"UPDATE USERS: Dati utente Wordpress ricevuti. (uuid="+uuid+").\n"});
					if(response.data && response.data.userMeta){
						var at = null; var rt = null; var exp = null;						
						if(Array.isArray(response.data.userMeta.sn_iml_at)){
							if(response.data.userMeta.sn_iml_at.length>0){
								at = response.data.userMeta.sn_iml_at[0];
							}
						}
						else{
							at = response.data.userMeta.sn_iml_at;
						}

						if(Array.isArray(response.data.userMeta.sn_iml_rt)){
							if(response.data.userMeta.sn_iml_rt.length>0){
								rt = response.data.userMeta.sn_iml_rt[0];
							}
						}
						else{
							rt = response.data.userMeta.sn_iml_rt;
						}

						if(Array.isArray(response.data.userMeta.sn_iml_token_expires_in)){
							if(response.data.userMeta.sn_iml_token_expires_in.length>0){
								exp = parseInt(response.data.userMeta.sn_iml_token_expires_in[0]);
							}
						}
						else{
							exp = parseInt(response.data.userMeta.sn_iml_token_expires_in);
						}

						var token_last_date = 0;
						if(Array.isArray(response.data.userMeta.sn_iml_token_last_date)){
							if(response.data.userMeta.sn_iml_token_last_date.length>0){
								token_last_date = parseInt(response.data.userMeta.sn_iml_token_last_date[0]);
							}
						}
						else{
							token_last_date = parseInt(response.data.userMeta.sn_iml_token_last_date);
						}
						...
						if(expire_time<now){
							mThis.updateUuidToUpdateLog({text:"UPDATE USERS: Token scaduto (last_date="+token_last_date+",exp="+exp+",expire_time="+expire_time+"), nuova richiesta token. (uuid="+uuid+").\n"});
						...
						mThis.updateUuidToUpdateLog({text:"UPDATE USERS: ERRORE User meta non disponibili (uuid="+uuid+").\n", isError: true});
						mCallback();
					}
				}
				else{
					mThis.updateUuidToUpdateLog({text:"UPDATE USERS: ERRORE Non è stato possibile trovare l'utente (uuid="+uuid+").\n", isError: true});
					mCallback();
				}
				/*
				mThis.updatePendingUserRecursiveAjaxCall({
			...
			mThis.updateUuidToUpdateLog({text:"UPDATE USERS: ERRORE errore ajax user_report_update_pending (uuid="+uuid+").\n\n", isError: true});
			mCallback();
		});
	},
	...
	onRefreshTokenSuccess: function(result, params){
		...
		SnImlApiClient.getUserData(getUserDataParams);
		this.updateUuidToUpdateLog({text:"UPDATE USERS: Token aggiornato. (uuid="+uuid+").\n"});
	},
	onRefreshTokenError: function(error, params){
		...
		this.updateUuidToUpdateLog({
			text: "UPDATE USERS: ERRORE Token non aggiornato, l'utente deve rieffettuare il login (uuid="+uuid+") (possibili cause: auth fallita o refreshtoken errato).\n", 
			isError: true
		});
	...
	onGetUserDataSuccess: function(params){
		params = params || {};
		var uuid = params.uuid || {};
		this.updateUuidToUpdateLog({text:"UPDATE USERS: Dati Itsme ricevuti (uuid="+uuid+").\n"});
	},
	onGetUserDataUserNotSubscribed: function(params){
		params = params || {};
		var uuid = params.uuid || {};
		this.updateUuidToUpdateLog({text:"UPDATE USERS: Utente NON sottoscritto a questo customer service, i dati itsme verranno cancellati (uuid="+uuid+").\n"});
	},
	onGetUserDataError: function(error, params){
		params = params || {};
		var uuid = params.uuid || {};
		console.log("onGetUserDataError");
		this.updateUuidToUpdateLog({text:"UPDATE USERS: ERRORE Dati Itsme NON ricevuti (uuid="+uuid+").\n", isError: true});
		//probably token has expired
	},
	...
	onElaborateLoginSuccess: function(params){
		...
		this.updateUuidToUpdateLog({text:"UPDATE USERS: Dati utente wordpress aggiornati. (uuid="+uuid+").\n"});
	},
	onElaborateLoginError: function(error, params){
		params = params || {};
		var uuid = params.uuid || {};
		console.log("onElaborateLoginError");	
		this.updateUuidToUpdateLog({text:"ERRORE Dati utente wordpress NON aggiornati.  (uuid="+uuid+").\n", isError: true});
	},
